...
function deleteToDoList(){
     ...
     <div class="card mt-2">
       <div class="d-flex justify-content-center">
        <input 
          ...
          style="width:30px"
          value="${(output.id)}"
          onchange="onChangeCompleteTask('${output?.title}')"
        >
        <h1 class="text-center text-info text-truncate" id="checkCompletedTask">${output.title}</h1>
 ...
 function onChangeCompleteTask(title){
   let isCompleted = window.confirm("Are you sure that you have completed it?")
   if(isCompleted === true){
     document.querySelector("#checkCompletedTask").innerHTML = `<del class="text-danger">${title}</del>`
   }else{
